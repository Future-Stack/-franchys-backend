...
export class EmailTrackerController {
  ...
  @Public()
  @Post('webhook')
  @ApiOperation({ summary: 'Webhook to receive Gmail notifications', description: 'Used by Google Cloud Pub/Sub to notify the app of new emails (Internal Use).' })
  ...
  @Get('test-watch')
  @ApiOperation({ summary: 'Trigger Gmail Watch', description: 'Manually triggers the Gmail watch subscription to start receiving notifications.' })
  @ApiResponse({ status: 200, description: 'Gmail watch triggered successfully.' })
  ...
  @Get('threads')
  @ApiOperation({ summary: 'Get all email threads', description: 'Retrieves a list of all email threads ordered by latest activity.' })
  ...
  @ApiOperation({ summary: 'Get messages for a thread', description: 'Retrieves all messages belonging to a specific thread ID.' })
  ...
  @ApiOperation({ summary: 'Reply to an email thread', description: 'Sends a direct email reply to the specified thread ID.' })
  @ApiParam({ name: 'id', description: 'The unique ID of the thread to reply to' })
  @ApiBody({ type: ReplyDto })
  @ApiResponse({ status: 201, description: 'Reply sent successfully.' })
  async replyToThread(@Param('id') threadId: string, @Body() replyDto: ReplyDto) {
